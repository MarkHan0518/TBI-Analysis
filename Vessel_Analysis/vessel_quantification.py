...
# Set Parameters
save_path = ''
file_name = '' # must be a binary 3D dataset
sample_tag = 'test'
pixel_dims = [2.68, 2.68, 2.68] # in um/px


bin_image = imread(save_path + file_name)
# Initiate Parameters
info_dict = {
    'pruning count': 0,
    'filaments count': 0,
...
# Propagate radius to the entire vessel
expanded_centerline = functions.custom_dilation_by_radius(bin_image, centerlines, radius_matrix)
expanded_centerline = np.transpose(expanded_centerline, (2, 1, 0))
imsave(save_path + 'expanded_centerline.tif', expanded_centerline)
...
with open(save_path + "segments_dict.pkl", 'wb') as file:
    pickle.dump(segments_dict, file)
