...
def custom_dilation_by_radius(bin_image: np.ndarray, centerlines: np.ndarray, radius_matrix: np.ndarray) -> np.ndarray:
    """
    Expands centerlines in a binary image based on varying radius values.

    Parameters
    ----------
    bin_image : np.ndarray
        A binary image where the centerlines are to be expanded.
    centerlines : np.ndarray
        An ndarray representing the centerlines to be expanded.
    radius_matrix : np.ndarray
        An ndarray containing radius values for each voxel.

    Returns
    ----------
    np.ndarray
        A binary image with expanded centerlines, where each voxel's value is scaled by its radius.
    """
    # Change the dtype to uint8 to reduce the data size
    expanded_centerline = np.zeros_like(bin_image, dtype=np.uint8)
    
    # Get unique radius values and convert them to uint8
    unique_radii = np.unique(radius_matrix).astype(np.uint8)
    unique_radii = unique_radii[unique_radii > 0]  # Exclude zero radius if present
    
    for radius in unique_radii:
        if radius > 0:
            # Create a cubic structuring element of size [radius, radius, radius]
            struct_elem = np.ones((radius, radius, radius), dtype=bool)
            
            # Get the coordinates where the radius is equal to the current radius
            mask = np.zeros_like(bin_image, dtype=bool)
            mask[radius_matrix == radius] = True
            
            # Perform binary dilation
            dilated = ndi.binary_dilation(mask, structure=struct_elem)
            
            # Combine the dilated structure with the expanded centerline
            expanded_centerline = np.maximum(expanded_centerline, dilated.astype(np.uint8) * radius)
    
    return expanded_centerline
